...
### 4.1 老师工作台 (Teacher Workspace)

老师端作为数据监控与教学反思工具，以最简洁的数据表和线条图呈现，避免图表混乱。
...
#### 4.1.2 页面布局结构与核心组件
*   **顶层指标行**：`grid grid-cols-4 gap-6 mb-8`。展示诸如“活跃学生数”、“平均任务完成率”，均采用超大黑色粗体数字配上柔和灰色的说明。
*   **学生概览表**：`minimal-card p-6`。清爽列表列出学生基本姓名、学号、今日累积时长、待审批任务数。
*   **迷你贡献热力图矩阵**：列表中每一个学生的右侧，直观内嵌一个缩小版的 12 周活跃热力图（Sparkline Heatmap），一眼便能看出全班学生最近的学习趋势。
